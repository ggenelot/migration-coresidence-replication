...
final_data_combined$id_mun <- as.numeric(interaction(data$ENT, data$MUN))
final_data_combined$id_panelgroup <- as.numeric(interaction(data$panel_group))
```
...
final_data_combined$nonhhnuclear_q1 <- ifelse(data$hhnuclear_q1 == 0, 1, 0)
attr(data$nonhhnuclear_q1, "label") <- "non-nuclear household q1"
...
for (place in c("all", "urb", "rur")) {
  ...
  for (i in 1:2) {
    # OLS regression
    ols_model <- lm(HHnewmember_allperiod ~ HHusmig_allperiod + HHusmig_fem_allperiod + i.id_panelgroup + control, data = subset(final_data_combined, samplee))
    ...
    fe_model <- lm(HHnewmember_allperiod ~ HHusmig_allperiod + HHusmig_fem_allperiod + i.id_panelgroup + control, data = subset(final_data_combined, samplee))
