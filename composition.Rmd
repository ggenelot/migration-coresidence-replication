...
for (place in c("all", "urb", "rur")) {
  ...
  for (i in 1:2) {
    # OLS regression
    ols_model <- lm(HHnewmember_allperiod ~ HHusmig_allperiod + HHusmig_fem_allperiod + id_panelgroup, data = subset(final_data_combined, samplee)) #erreur ici, j'essaie de retirer control
    ...
    fe_model <- lm(HHnewmember_allperiod ~ HHusmig_allperiod + HHusmig_fem_allperiod + id_panelgroup + control, data = subset(final_data_combined, samplee))
    ...
    fe_results <- rbind(fe_results, cbind(fe_output[, 1], rownames(fe_output)))
...
for (place in c("all", "urb", "rur")) {
  ...
  # Define control variables
  control <- c("i.hhmaxedu_q1")
  
  # Perform regressions and output tables
  for (i in 1:2) {
    # OLS regression
    ols_model <- lm(HHnewmember_allperiod ~ HHusmig_allperiod + HHusmig_fem_allperiod + id_panelgroup + control, data = subset(final_data_combined, samplee))
    ols_output <- coeftest(ols_model, vcov = vcovHC(ols_model, cluster = ~id_h))
    ...
    # ne marche pas avec i.id_panelgroup, j essaie avec juste id_panelgroup
